...
$(document).ready(function(){
	let alerts;
	$(".product_wrap li").click(function(){
		$(this).addClass("product_checked").siblings().removeClass("product_checked")
	})
	
	$(".property_wrap .property ul li").click(function(){
		$(this).addClass("property_checked").siblings().removeClass("property_checked")
	})
    ...
    data1.map(item=>{
        item.children.map(val=>{
            val.content.split(',').map((v,index)=>{
                if(index>0){
                    val.children.push({name:v})
                }
            })
        })
    })
  ...
  //初始化第一个option
	...
	data1.map(item=>{
        province1.innerHTML+=`<option value="${item.name}">${item.name}</option>`	
	
    })
	//选择第一个后初始化第二个option，并且清空第三个option
	
    province.onchange=function (){       	
    	province.selected="selected"
    	city.innerHTML=`<option value="选城市">选城市</option>`
        area.innerHTML=`<option value="选地区">选地区</option>`
        const obj=data1.find(item=>province.value===item.name)
        if(!obj)return
	...
	province1.onchange=function (){       	
    	province1.selected="selected"
    	city1.innerHTML=`<option value="选城市">选城市</option>`
        area1.innerHTML=`<option value="选地区">选地区</option>`
        const obj=data1.find(item=>province1.value===item.name)
        if(!obj)return
        obj.children.map(item=>{
            city1.innerHTML+=`<option value="${item.name}">${item.name}</option>`
        })
	}
    ...
    city.onchange=function(){
    	...
    	area.innerHTML=`<option value="选地区">选地区</option>`
        const obj=data1.find(item=>province.value===item.name).children.find(item=>city.value===item.name)
        obj.children.map(item=>{
            area.innerHTML+=`<option value="${item.name}">${item.name}</option>`
        })
	}
	city1.onchange=function(){
    	city1.selected="selected"
    	area1.innerHTML=`<option value="选地区">选地区</option>`
        const obj=data1.find(item=>province1.value===item.name).children.find(item=>city1.value===item.name)
        obj.children.map(item=>{
            area1.innerHTML+=`<option value="${item.name}">${item.name}</option>`
        })
    }
	...
	$(".submit").click(function(){
		...
		let userMob = $("#mobile").val()
		let userProvince = $("#province option:selected").val()
		let userCity = $("#city option:selected").val()
		let userArea = $("#area option:selected").val()
